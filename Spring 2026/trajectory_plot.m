% function [M]=trajectory_plot(x,y,z,roll,pitch,yaw,scale_factor,step,selector,varargin);
function [M]=trajectory_plot(pos,att,scale_factor,step,selector,varargin);
%   pos (x,y,z)         center trajectory (vector)    [m]
%   
%   att (roll,pitch,yaw) euler's angles               [rad]
%   
...
Tbe=[ cos(theta)*cos(psi) sin(phi)*sin(theta)*cos(psi)-cos(phi)*sin(psi) cos(phi)*sin(theta)*cos(psi)+sin(phi)*sin(psi);
